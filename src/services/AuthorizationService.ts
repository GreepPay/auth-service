...
export class AuthorizationService {
     ...
     */
    async updatePermissionInRole(data: UpdatePermissionForm): Promise<string | HttpResponseType> {
        const role = await Role.findOne({ where: { uuid: data.role_uuid }});
            ...
            for (const value of subKeyOptions) {
                ...
                if (currentPermission) {
                    await Permission.update(
                        ...
                        {
                            status: permissionGroup[value as keyof typeof permissionGroup] as boolean,
                        }
                    );
                } else {
    ...
    async userCan(permission_name: string): Promise<boolean | HttpResponseType> {
        const user = await User.findOne({ where: { uuid: this.jwtService.getCurrentUserId() } });
        const permission = user?.role?.permissions?.find(permission => permission.name === permission_name);
