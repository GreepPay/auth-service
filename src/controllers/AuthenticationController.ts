import type { AuthenticateUserResponse, ResetPasswordForm, UpdateUserProfileForm, ValidateOtpForm } from './../forms/authentication';
import type { AuthenticateUserForm, CreateUserForm } from '../forms/authentication';
...
export class AuthenticationController {
  ...
  async login(request: BunRequest) {
    let data: AuthenticateUserForm = await request.json() as AuthenticateUserForm;
    let response = await new AuthenticationService(request).authenticateUser(data);
  ...
  async resetUserOtp(request: BunRequest) {
    ...
    let response = await new AuthenticationService(request).resetUserOtp(data.email);
  ...
  async updateUserPassword(request: BunRequest) {
    let data: ResetPasswordForm = await request.json() as ResetPasswordForm;
    let response = await new AuthenticationService(request).updateUserPassword(data);
  ...
  async updateUserProfile(request: BunRequest) {
    let data: UpdateUserProfileForm = await request.json() as UpdateUserProfileForm;
    let response = await new AuthenticationService(request).updateUserProfile(data);
